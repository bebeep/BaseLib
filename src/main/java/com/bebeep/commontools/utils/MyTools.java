...
public class MyTools {
    ...
    public static String formateTime(String time){
        ...
        long dTime = (time2 - time1)/1000;
        if(dTime<5){
            return "刚刚";
        }else if(dTime<60){//1分钟以内
            return dTime+"秒前";
        }else if(dTime>=60&&dTime<3600){//1小时以内
            return dTime/60+"分钟前";
        }else if(dTime>=3600&&dTime<86400){//1天以内
            if(dTime%3600<1800){
                return dTime/3600+"小时前";
            }else{
                return (dTime/3600+1)+"小时前";
            }
        }else if(dTime>=86400&&dTime<259200){//3天以内
 ...
 */
    ...
    public static String getCity(Context context) {
        ...
        return preferences.getString("city", "宜宾市");
    }
...
//        MyToast.makeText(context, content, Toast.LENGTH_SHORT).show();
